...
app.post('/api/payments/mpesa/stk-push', payLimiter, async (req, res) => {
  const { phone, plan='pro', amount } = req.body;
  if (!phone) return res.status(400).json({ success:false, message:'Phone number required (format: 254XXXXXXXXX)' });

  if (!process.env.MPESA_CONSUMER_KEY) {
    return res.json({ success:false, message:'M-Pesa not configured. Add MPESA_CONSUMER_KEY, MPESA_CONSUMER_SECRET, MPESA_SHORTCODE, MPESA_PASSKEY to .env', setup_url:'https://developer.safaricom.co.ke', demo:true });
  }

  const planData = PLANS[plan];
  const amountKES = amount || planData?.priceKES || 3800;
  const base = process.env.MPESA_SANDBOX === 'true' ? 'https://sandbox.safaricom.co.ke' : 'https://api.safaricom.co.ke';
  ...
  try {
    ...
    const r = await axios.post(`${base}/mpesa/stkpush/v1/processrequest`, {
      ...
      PhoneNumber:       phone,
      CallBackURL:       `${process.env.BACKEND_URL || 'https://growthpilot-api.onrender.com'}/api/payments/mpesa/callback`,
      AccountReference:  `GrowthPilot ${planData?.name || plan}`,
    ...
    console.log(`📱 M-Pesa STK Push sent to ${phone} — KES ${amountKES}`);
    return res.json({ success:true, message:`M-Pesa prompt sent to ${phone}! Enter your PIN to complete payment.`, checkoutRequestId:checkoutId, amount:amountKES, currency:'KES' });
  } catch(err) {
    return res.status(500).json({ success:false, message:`M-Pesa error: ${err.response?.data?.errorMessage || err.message}` });
  }
});
